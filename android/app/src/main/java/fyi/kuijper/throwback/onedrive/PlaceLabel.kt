 ...
 *
 * Only the street name is used — the house number is intentionally omitted. Country is included only
 * when it differs from [homeCountryCode]; junk street values ("Unnamed Road") are dropped; null when
...
internal object PlaceLabel {
    ...
    fun compose(
        ...
        val street = thoroughfare.trimToNull()?.takeUnless { it.lowercase() in JUNK_STREETS }
